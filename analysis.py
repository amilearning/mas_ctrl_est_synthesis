...
class Analysis:
    ...
    def get_df(self,stage_costs, num_list, algorithm_name):        
        df = pd.DataFrame({
            ...
            'Algorithm': [algorithm_name] * len(stage_costs[0])
        })
        return df 
    ...
    def draw_boxplot(self):
        ...
        merged_df = pd.concat([lqr_df, sub_df, opt_df], ignore_index=True)


        
        df = pd.DataFrame({'FullyconnectedLQR': stage_costs_lqr, 'Suboptimal': stage_costs_sub, 'Proposed': stage_costs_opt})               
